...
`gtm-churn-prevention` (triggered by risk signals) and
`gtm-upsell-expansion` (triggered by an opportunity). Use this one for the
...
```
claude "run gtm-customer-success on ./calls/acme-qbr.txt"
```
...
```
claude "run gtm-customer-success on ./exports/accounts.csv"
```
