...
`evals/baseline-*.json`, `evals/runs/*`, `evals/runs.jsonl`, and
`evals/amendments.jsonl` are all gitignored — real client names, deal
details, and rubric-fix proposals derived from them live in several of
these. Only `evals/framework/*`, `evals/promptfooconfig.yaml`, and
`evals/cases.example.yaml` are public. `evals/gt-web/` (see
`evals/gt-web/README.md`) is a separate, third-party-sourced GT
mechanism — the harvested excerpts themselves are gitignored too
(copyright, not client-confidentiality, is the reason there), but its
`SOURCES.md` citation registry is deliberately public so a claimed source
can be checked without redistributing it. `./validate-skills.sh` checks
the `.gitignore` entries are still in place.
